...
- (void)setUI {
    ...
    [self.extractButton mas_remakeConstraints:^(MASConstraintMaker *make) {
        make.width.offset(AUTOLAYOUTSIZE(25.5));
        make.height.offset(AUTOLAYOUTSIZE(25.5));
...
- (void)setModel:(DBHWalletDetailTokenInfomationModelData *)model {
    ...
    if ([_model.flag isEqualToString:@"NEO"] || [_model.flag isEqualToString:@"Gas"]) {
        ...
        self.numberLabel.text = [NSString stringWithFormat:@"%.4lf", _model.balance.floatValue];
    ...
    if (_model.canExtractbalance) {
        self.canExtractGasLabel.text = [NSString stringWithFormat:@"（%.8lfGas可提取）", _model.canExtractbalance.floatValue];
    }
}
...
- (UIButton *)extractButton {
    if (!_extractButton) {
        ...
        _extractButton.imageView.contentMode = UIViewContentModeScaleAspectFit;
        [_extractButton setImage:[UIImage imageNamed:@"Group 5"] forState:UIControlStateNormal];
