...
- (void)respondsToShowAssetButton {
    ...
    self.assetLabel.text = [NSString stringWithFormat:@"%@：%@%@", NSLocalizedString(@"Total Balance", nil), [UserSignData share].user.walletUnitType == 1 ? @"¥" : @"$", [UserSignData share].user.isHideAsset ? @"****" : self.totalAsset];
}
...
- (UIButton *)showAssetButton {
    if (!_showAssetButton) {
        ...
        [_showAssetButton setImage:[UIImage imageNamed:@"闭眼"] forState:UIControlStateNormal];
        [_showAssetButton setImage:[UIImage imageNamed:@"睁眼1"] forState:UIControlStateSelected];
