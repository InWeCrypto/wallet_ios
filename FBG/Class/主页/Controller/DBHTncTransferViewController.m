...
@interface DBHTncTransferViewController ()<MailListDelegate, ScanVCDelegate, UITextFieldDelegate>
...
- (BOOL)textField:(UITextField *)textField shouldChangeCharactersInRange:(NSRange)range replacementString:(NSString *)string {
    ...
    if (changeAfterString.length && changeAfterString.floatValue > 0) {
        ...
        self.poundageLabel.text = [NSString stringWithFormat:@"%@：0.00 Gas", NSLocalizedString(@"Transaction Fee", nil)];
    }
...
- (void)getPoundageWithAcount:(NSString *)Acount {
    ...
    WEAKSELF
    [PPNetworkHelper POST:@"extend/getNeoGasCost" parameters:paramters hudString:nil success:^(id responseObject) {
        weakSelf.poundage = [NSString stringWithFormat:@"%@", responseObject[@"gas_consumed"]];
        weakSelf.poundageLabel.text = [NSString stringWithFormat:@"%@：%.2lf Gas", NSLocalizedString(@"Transaction Fee", nil), weakSelf.poundage.floatValue];
 ...
 */
- (void)checkGasCanUse {
    self.commitButton.userInteractionEnabled = self.canUseGasBalance.floatValue && self.canUseGasBalance.floatValue >= self.poundage.floatValue;
    self.commitButton.backgroundColor = !self.commitButton.userInteractionEnabled ? COLORFROM16(0xDADADA, 1) : COLORFROM16(0xFF841C, 1);
    [self.commitButton setTitle:NSLocalizedString(!self.commitButton.userInteractionEnabled ? @"The current wallet GAS balance is insufficient and cannot be submitted" : @"Commit", nil) forState:UIControlStateNormal];
}
...
- (UILabel *)poundageLabel {
    if (!_poundageLabel) {
        ...
        _poundageLabel.text = [NSString stringWithFormat:@"%@：0.00 Gas", NSLocalizedString(@"Transaction Fee", nil)];
...
- (UILabel *)noPoundageLabel {
    if (!_noPoundageLabel) {
        ...
        _noPoundageLabel.text = NSLocalizedString(@"（Free handling charge under 10Gas）", nil);
...
- (UILabel *)canUseGasNumberLabel {
    if (!_canUseGasNumberLabel) {
        ...
        NSMutableAttributedString *balanceAttributedString = [[NSMutableAttributedString alloc] initWithString:[NSString stringWithFormat:@"（Gas %@：%@）", NSLocalizedString(@"Available Number", nil), balance]];
        [balanceAttributedString addAttribute:NSForegroundColorAttributeName value:COLORFROM16(0xF9480E, 1) range:NSMakeRange([NSString stringWithFormat:@"Gas %@", NSLocalizedString(@"Available Number", nil)].length + 2, balance.length)];
