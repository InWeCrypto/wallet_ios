...
- (void)viewDidLoad {
    ...
    self.title = NSLocalizedString(@"Token Sale Confirmation", nil);
...
- (void)setUI {
    ...
    self.poundageLabel.text = [NSString stringWithFormat:@"%@：0.000000000", NSLocalizedString(@"Poundage", nil)];
    self.originalPoundageLabel.text = [NSString stringWithFormat:@"（%@：3.00000000Gas）", NSLocalizedString(@"Original Poundage", nil)];
...
- (UILabel *)collectionAddressLabel {
    if (!_collectionAddressLabel) {
        ...
        _collectionAddressLabel.text = NSLocalizedString(@"Intelligent Contract Address", nil);
...
- (UIButton *)sureButton {
    if (!_sureButton) {
        ...
        [_sureButton setTitle:NSLocalizedString(@"Commit", nil) forState:UIControlStateNormal];
