...
- (void)viewDidLoad {
    ...
    self.title = NSLocalizedString(@"Extract Gas", nil);
...
- (void)respondsToSureButton {
    if (self.gasTokenModel.balance.floatValue <= 0) {
...
- (DBHInputPasswordPromptView *)inputPasswordPromptView {
    if (!_inputPasswordPromptView) {
        ...
        [_inputPasswordPromptView commitBlock:^(NSString *password) {
            [LCProgressHUD showLoading:NSLocalizedString(weakSelf.type == 1 ? @"In gas extraction, Please be patient" : @"In the validation...", nil)];
            ...
            dispatch_async(globalQueue, ^
                           {
                               if (weakSelf.type == 1) {
                                   // 解冻
                                   [PPNetworkHelper GET:[NSString stringWithFormat:@"extend/getNeoUtxo?address=%@&type=%@", weakSelf.gasTokenModel.address, @"neo-asset-id"] isOtherBaseUrl:NO parameters:nil hudString:@"" success:^(id responseObject) {
                                   ...
                                   // 提取
                                   [PPNetworkHelper GET:[NSString stringWithFormat:@"extend/getNeoClaimUtxo?address=%@", weakSelf.gasTokenModel.address] isOtherBaseUrl:NO parameters:nil hudString:@"" success:^(id responseObject) {
