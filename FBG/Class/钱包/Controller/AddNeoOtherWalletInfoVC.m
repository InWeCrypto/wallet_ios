...
@implementation AddNeoOtherWalletInfoVC
...
- (void)sureWithPassWord:(NSString *)passWord
{
    //确认支付密码
    //keyStore
    NSString *data = self.infoTextView.text;
    ...
    dispatch_async(globalQueue, ^
                   {
                       ...
                       self.wallet = NeomobileFromKeyStore(data,passWord,&error);
                       ...
                       dispatch_async(mainQueue, ^
                                      {
                                          if (!error)
                                          {
                                              ...
                                              {
                                                  //观察钱包升级 keyStore
                                                  if ([[self.walletModel address] isEqualToString:self.walletModel.address])
                                                  {
                                                      [LCProgressHUD showMessage:@"转化成功"];
                                                      [PDKeyChain save:[self.wallet address] data:data];
            ...
            dispatch_async(globalQueue, ^
                           {
                               //子线程异步执行下载任务，防止主线程卡顿
                               NSError * error;
                               self.wallet = NeomobileFromMnemonic(self.infoTextView.text,&error);
            ...
            dispatch_async(globalQueue, ^
                           {
                               //子线程异步执行下载任务，防止主线程卡顿
                               NSError * error;
                               self.wallet = NeomobileFromWIF(self.infoTextView.text,&error);
