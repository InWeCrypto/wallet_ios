...
@interface AddNeoWalletInfoVC () <AlertViewDelegate>
...
@implementation AddNeoWalletInfoVC
...
- (IBAction)sureAddButtonCilick:(id)sender
{
    ...
    if (self.neoWallet)
    {
        ...
        if ([UserSignData share].user.isCode)
        {
            ...
            model.address = [[[self.neoWallet address] lowercaseString] lowercaseString];
            int y = 1 +  (arc4random() % 11);
            model.img = [NSString stringWithFormat:@"钱包头像%d",y];
            //本分本地
            NSMutableDictionary * parametersDic = [[NSMutableDictionary alloc] init];
            [parametersDic setObject:[[[self.neoWallet address] lowercaseString] lowercaseString] forKey:@"address"];
        ...
        else
        {
            ...
            if (self.walletModel)
            {
                //转化
                NSError * error;
                NSString *data = [self.neoWallet toKeyStore:self.pasWordTF.text error:&error];
            ...
            else
            {
                ...
                [parametersDic setObject:self.nameTF.text forKey:@"name"];
                [parametersDic setObject:[self.neoWallet address] forKey:@"address"];
                           ...
                           dispatch_async(mainQueue, ^
                                          {
                                              ...
                                              if (!error)
                                              {
                                                  [LCProgressHUD hide];
                                                  NSError * error;
                                                  self.mnemonicStr = [self.neoWallet mnemonic:&error];
                                                  if (!error)
                                                  {
                                                      //提醒助记词
//                                                      [self showAlert];
                       ...
                       dispatch_async(mainQueue, ^
                                      {
                                          if (!error)
                                          {
                                              ...
                                              if ([UserSignData share].user.isCode)
                                              {
                                                  ...
                                                  model.name = self.nameTF.text;
                                                  model.category_name = @"ETH";
                                                  model.address = [self.neoWallet address];
                                                  model.isNotBackupsWallet = YES;
                                                  int y = 1 +  (arc4random() % 11);
                                                  model.img = [NSString stringWithFormat:@"钱包头像%d",y];
                                                  //本分本地
