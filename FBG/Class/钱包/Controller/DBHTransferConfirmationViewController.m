    ...
    dispatch_async(mainQueue, ^
                   {
                       if (!error)
                       {
                           if (![self.tokenModel.name isEqualToString:@"ETH"])
                           {
                               //代币钱包转账
...
//                               {
                                   ...
                                   [PPNetworkHelper POST:@"extend/transferABI" baseUrlType:1 parameters:parametersDic hudString:@"加载中..." success:^(id responseObject)
                                    {
                                        //生成data
                                        NSError * error;
...
//                               }
                           ...
                           else
                           {
                               ...
                               //子线程异步执行下载任务，防止主线程卡顿
                               NSError * error;
                               NSString * price = [SystemConvert decimalToHex:[NSString DecimalFuncWithOperatorType:2 first:self.transferNumber secend:@"1000000000000000000" value:0].integerValue];
                               ...
                               NSString *data = [Wallet transfer:self.nonce
                                                         ...
                                                         amount:[NSString stringWithFormat:@"0x%@", price]
...
//                                                      {
                                                          //热钱包生成订单
                                                          [self creatOrderWithData:[NSString stringWithFormat:@"0x%@",data] asset_id:@"0x0000000000000000000000000000000000000000"];
//                                                      }
                                                  }
                                                  else
                                                  {
    ...
    dispatch_async(mainQueue, ^
                   {
                       if (!error)
                       {
                           ...
                           //                           {
                           //热钱包代币转账
                           NSString * price = [NSString stringWithFormat:@"0x%@",[SystemConvert decimalToHex:[[NSString stringWithFormat:@"%.0f000000000000",[self.transferNumber floatValue] * 100000000] longLongValue]]];
                           NSMutableDictionary * parametersDic = [[NSMutableDictionary alloc] init];
                           [parametersDic setObject:self.neoWalletModel.address forKey:@"contract"];
                           [parametersDic setObject:self.address forKey:@"to"];
                           [parametersDic setObject:price forKey:@"value"];
...
- (void)creatOrderWithData:(NSString *)data asset_id:(NSString *)asset_id
{
    ...
    [dic setObject:[NSString stringWithFormat:@"0x%@", [NSString getHexByDecimal:[NSString DecimalFuncWithOperatorType:2 first:self.transferNumber secend:@"1000000000000000000" value:0].integerValue]] forKey:@"fee"];
    ...
    [PPNetworkHelper POST:@"wallet-order" baseUrlType:1 parameters:dic hudString:@"创建中..." success:^(id responseObject)
     {
         //进入订单详情
         /*
...
- (void)creatNeoOrderWithData:(NSString *)data trade_no:(NSString *)trade_no
{
    ...
    [dic setObject:self.remark forKey:@"remark"];
    [dic setObject:[self.tokenModel.flag isEqualToString:@"TNC"] ? [NSString stringWithFormat:@"%lf", self.transferNumber.floatValue * pow(10, self.tokenModel.decimals.floatValue)] : self.transferNumber forKey:@"fee"];
...
- (void)getUnspentWithPassword:(NSString *)password {
    ...
    if ([self.tokenModel.flag isEqualToString:@"Gas"]) {
        [PPNetworkHelper GET:[NSString stringWithFormat:@"extend/getNeoUtxo?address=%@&type=%@", weakSelf.neoWalletModel.address, @"neo-gas-asset-id"] baseUrlType:3 parameters:nil hudString:@"" success:^(id responseObject) {
        ...
        [PPNetworkHelper GET:[NSString stringWithFormat:@"extend/getNeoUtxo?address=%@&type=%@", weakSelf.neoWalletModel.address, @"neo-asset-id"] baseUrlType:1 parameters:nil hudString:@"" success:^(id responseObject) {
            NSArray *result = responseObject[@"result"];
            [weakSelf transferAccountsForNEOWithPassword:password unspent:[result toJSONStringForArray]];
        } failure:^(NSString *error) {
            [LCProgressHUD showFailure:error];
        ...
        [PPNetworkHelper GET:[NSString stringWithFormat:@"extend/getNeoUtxo?address=%@&type=%@", self.neoWalletModel.address, @"neo-asset-id"] baseUrlType:1 parameters:nil hudString:@"" success:^(id responseObject) {
            [result addObjectsFromArray:responseObject[@"result"]];
            [PPNetworkHelper GET:[NSString stringWithFormat:@"extend/getNeoUtxo?address=%@&type=%@", weakSelf.neoWalletModel.address, @"neo-gas-asset-id"] baseUrlType:1 parameters:nil hudString:@"" success:^(id responseObject) {
                [result addObjectsFromArray:responseObject[@"result"]];
...
- (UILabel *)numberLabel {
    if (!_numberLabel) {
        ...
        _numberLabel.text = [NSString stringWithFormat:@"%.4lf", self.transferNumber.floatValue];
...
- (UILabel *)poundageLabel {
    if (!_poundageLabel) {
        ...
        if ([self.tokenModel.flag isEqualToString:@"TNC"]) {
            _poundageLabel.text = [NSString stringWithFormat:@"%@：%lf Gas", DBHGetStringWithKeyFromTable(@"Fees", nil), self.realityPoundage.floatValue];
        } else if ([self.tokenModel.flag isEqualToString:@"NEO"] || [self.tokenModel.flag isEqualToString:@"Gas"]) {
            _poundageLabel.text = [NSString stringWithFormat:@"%@：%.4lf", DBHGetStringWithKeyFromTable(@"Fees", nil), self.poundage.floatValue];
        } else {
            _poundageLabel.text = [NSString stringWithFormat:@"%@：%.8lf", DBHGetStringWithKeyFromTable(@"Fees", nil), self.poundage.floatValue];
        }
...
- (UILabel *)originalPoundageLabel {
    if (!_originalPoundageLabel) {
        ...
        _originalPoundageLabel.hidden = ![self.tokenModel.flag isEqualToString:@"TNC"];
        _originalPoundageLabel.text = [NSString stringWithFormat:@"（%@：%lf Gas）", DBHGetStringWithKeyFromTable(@"Original Fees", nil), self.poundage.floatValue];
    ...
    if (!_inputPasswordPromptView) {
        ...
        [_inputPasswordPromptView commitBlock:^(NSString *password) {
            ...
            dispatch_async(globalQueue, ^
                           {
                               if ([weakSelf.tokenModel.name isEqualToString:@"TNC"] || [weakSelf.tokenModel.name isEqualToString:@"Gas"] || [weakSelf.tokenModel.name isEqualToString:@"NEO"]) {
