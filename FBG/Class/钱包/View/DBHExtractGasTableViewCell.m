...
- (void)setGasTokenModel:(DBHWalletDetailTokenInfomationModelData *)gasTokenModel {
    ...
    self.maxExtractGasValueLabel.text = [NSString stringWithFormat:@"%.8lf", _gasTokenModel.canExtractbalance.floatValue];
    self.noExtractGasLabel.text = [NSString stringWithFormat:@"%@Gas：%.8lf", DBHGetStringWithKeyFromTable(@"Unfreeze", nil), _gasTokenModel.noExtractbalance.floatValue];
    self.canExtractGasLabel.text = [NSString stringWithFormat:@"%@1.000-%.8lf", DBHGetStringWithKeyFromTable(@"Can Extracted", nil),  _gasTokenModel.canExtractbalance.floatValue];
    self.extractGasTextField.text = [NSString stringWithFormat:@"%.8lf", _gasTokenModel.canExtractbalance.floatValue];
}
...
- (UIButton *)unfreezeButton {
    if (!_unfreezeButton) {
        ...
        _unfreezeButton.backgroundColor = [UIColor whiteColor];
...
- (UIView *)orangeLineView {
    if (!_orangeLineView) {
        ...
        _orangeLineView.backgroundColor = COLORFROM16(0xFF841C, 1);
    }
...
- (UILabel *)canExtractGasLabel {
    if (!_canExtractGasLabel) {
        ...
        _canExtractGasLabel.textColor = COLORFROM16(0xFF841C, 1);
    }
...
- (UIButton *)allExtractButton {
    if (!_allExtractButton) {
        ...
        [_allExtractButton setTitleColor:COLORFROM16(0xFF841C, 1) forState:UIControlStateNormal];
