...
- (void)addTitleLabels {
    for (NSInteger i = 0; i < self.titleArray.count + 1; i++) {
        ...
        titleLabel.frame = CGRectMake(!i ? 0 : CGRectGetMaxX([self.view viewWithTag:199 + i].frame), 0, [NSString getWidthtWithString:!i ? NSLocalizedString(@"All", nil) : model.name fontSize:AUTOLAYOUTSIZE(14)] + AUTOLAYOUTSIZE(42), AUTOLAYOUTSIZE(37));
        titleLabel.font = FONT(14);
        titleLabel.text = !i ? NSLocalizedString(@"All", nil) : model.name;
